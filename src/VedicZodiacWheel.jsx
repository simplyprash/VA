...
/* =============================================================
   Vedic Zodiac Wheel — FULL VERSION (clean build)
   - ASC only (blue) inside wheel
   - Vedic drishti with arrows
   - Two-letter planet labels inside wheel
   - Sidereal/tropical toggle, ayanamsha presets
   - CSV/PNG export, ErrorBoundary
   ============================================================= */
...
const NAK_SIZE = 360/27; const PADA_SIZE = NAK_SIZE/4;
function nakshatraOf(siderealLon:number){
  const lon = norm360(siderealLon); const idx = Math.floor(lon/NAK_SIZE);
  const within = lon - idx*NAK_SIZE; const pada = Math.floor(within/PADA_SIZE) + 1; const item = NAKSHATRAS[idx];
  return { index: idx, name: item.en, dev: item.dev, pada };
}

function zodiacBreakdown(longitudeDeg:number){
  const lon = norm360(longitudeDeg); const signIndex = Math.floor(lon/30); const inSign = lon % 30;
  const d = Math.floor(inSign); const m = Math.floor((inSign-d)*60);
...
function meanLunarNodeLongitude(date: Date){
  const JD = date.getTime()/86400000 + 2440587.5; const T = (JD - 2451545.0)/36525.0;
  const omega = 125.04455501 - 1934.13626197*T + 0.0020762*T*T + (T*T*T)/467410 - (T*T*T*T)/60616000;
  return norm360(omega);
}

/* --------------- ASC (numeric horizon solver) --------------- */
function lstRadians(date: Date, longitudeDeg:number){
  try{ const sth = Astronomy.SiderealTime(date); let lstDeg = sth*15 + (Number(longitudeDeg)||0); lstDeg = ((lstDeg%360)+360)%360; return lstDeg*DEG2RAD; }
  catch{ return 0; }
}

function computeAscendantDeg(date: Date, latitudeDeg:number, longitudeDeg:number){
  const φ = Math.max(-89.9999, Math.min(89.9999, Number(latitudeDeg)||0)) * DEG2RAD;
  const θ = lstRadians(date, Number(longitudeDeg)||0);
  ...
  const ε = epsDeg*DEG2RAD; const sinε=Math.sin(ε), cosε=Math.cos(ε); const sinφ=Math.sin(φ), cosφ=Math.cos(φ);
  function altAz(lambdaDeg:number){ const λ=lambdaDeg*DEG2RAD; const sinλ=Math.sin(λ), cosλ=Math.cos(λ); const α=Math.atan2(sinλ*cosε, cosλ); const δ=Math.asin(sinλ*sinε); let H=θ-α; H=Math.atan2(Math.sin(H), Math.cos(H)); const sinδ=Math.sin(δ), cosδ=Math.cos(δ); const sinAlt = sinφ*sinδ + cosφ*cosδ*Math.cos(H); const alt=Math.asin(Math.max(-1,Math.min(1,sinAlt))); const cosAlt = Math.max(1e-12, Math.sqrt(Math.max(0,1-sinAlt*sinAlt))); const sinAz = -Math.sin(H)*cosδ/cosAlt; const cosAz=(sinδ - sinAlt*sinφ)/(cosAlt*cosφ); let A=Math.atan2(sinAz, cosAz)/DEG2RAD; A=((A%360)+360)%360; return { alt, az:A }; }
  const roots:{lambda:number,az:number}[]=[]; let prev = altAz(0); let prevAlt = prev.alt; const step=5;
  for(let L=step; L<=360; L+=step){ const cur=altAz(L); const curAlt=cur.alt; if((prevAlt<=0 && curAlt>=0) || (prevAlt>=0 && curAlt<=0)){ let a=L-step, b=L, fa=altAz(a).alt, fb=altAz(b).alt; for(let k=0;k<30;k++){ const m=0.5*(a+b); const fm=altAz(m).alt; if(fa*fm<=0){ b=m; fb=fm; } else { a=m; fa=fm; } } const mid=0.5*(a+b); const atMid=altAz(mid); roots.push({lambda:mid, az:atMid.az}); } prevAlt = curAlt; }
  if(roots.length===0) return 0; roots.sort((r1,r2)=> Math.abs(r1.az-90)-Math.abs(r2.az-90)); return norm360(roots[0].lambda);
}
...
function planetLongitudes(date: Date, useMeanNode:boolean){
  const results:any[] = [];
  for (const item of BODIES) { const vec = Astronomy.GeoVector(item.body, date, true); const ecl = Astronomy.Ecliptic(vec); results.push({ key:item.key, color:item.color, body:item.body, elon:norm360(ecl.elon), optional:!!item.optional }); }
  const rahu = meanLunarNodeLongitude(date); const ketu = norm360(rahu + 180);
...
function resolveCollisions(points:any[], minSepDeg = 6){
  const sorted = [...points].sort((a,b)=>a.lon-b.lon);
  for(let i=1;i<sorted.length;i++){ const prev = sorted[i-1], cur = sorted[i]; const gap = Math.abs(norm360(cur.lon - prev.lon)); if(gap < minSepDeg) cur._bump = (prev._bump || 0) + 1; }
  if(sorted.length>1){ const first = sorted[0], last = sorted[sorted.length-1]; const wrapGap = Math.abs(norm360(first.lon + 360 - last.lon)); if(wrapGap < minSepDeg) first._bump = (last._bump || 0) + 1; }
  return points;
}

function angleToXY(angleDeg:number, r:number, cx:number, cy:number){ const a=(0-angleDeg)*DEG2RAD; return { x: cx + r*Math.cos(a), y: cy + r*Math.sin(a) }; }
    ...
    <div className="flex flex-col lg:flex-row gap-6">
      <svg width={size} height={size} className="rounded-2xl shadow border bg-white">
        ...
        {showDrishti && points.map((a:any,i:number) => {
          ...
          return points.map((b:any,j:number) => {
            if(i===j) return null; const bSign = Math.floor(b.lon/30); const dist = ((bSign - aSign + 12) % 12) + 1; if(!rules.includes(dist)) return null;
            ...
            const sx = pa.x + dx * (8/L), sy = pa.y + dy * (8/L); const ex = pb.x - dx * (12/L), ey = pb.y - dy * (12/L);
            return <line key={`dr-${i}-${j}`} x1={sx} y1={sy} x2={ex} y2={ey} stroke={a.color} strokeWidth={1.4} opacity={0.8} markerEnd="url(#arrowHead)" />;
          });
        })}
...
export default function VedicZodiacWheel(){
  ...
  const [showAspects, setShowAspects] = useState(false); // keep geometric off by default
  ...
  return (
    <ErrorBoundary>
      <div className="p-4 lg:p-6 font-sans text-slate-800">
        ...
        <p className="text-slate-600 mb-4">Sidereal option, nakshatras, ASC + drishti, compact labels, PNG/CSV export.</p>
        ...
        <div className="flex flex-wrap items-end gap-4 mb-4">
          ...
          <div className="flex gap-2 mt-6 flex-wrap">{AY_PRESETS.map(p => (
